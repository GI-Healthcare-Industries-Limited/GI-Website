...
export async function GET(request: Request) {
  ...
  const { data, error } = await getSupabaseAdmin().from('career_applications')
    .select('immigration_status, work_permission_declared, student_conditions_acknowledged')
    .eq('id', id.data).gt('retention_expires_at', new Date().toISOString()).maybeSingle()

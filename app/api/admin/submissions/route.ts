...
export async function GET(request: Request) {
  ...
  let query = kind.data === 'contact'
    ...
    : getSupabaseAdmin().from('career_applications').select('id, created_at, updated_at, retention_expires_at, privacy_notice_version, data_sharing_acknowledged_at, data_sharing_statement_version, job_title, name, email, phone, portfolio_url, project_summary, awards_status, competition_awards, award_entries, awards_detail, biggest_failure, growth_area, authorship_confirmed_at, application_questions_version, right_to_work, cover_letter, status')
